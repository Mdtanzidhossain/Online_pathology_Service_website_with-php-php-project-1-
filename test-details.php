...
$query=mysqli_query($con,"update covidtestrecord set ReportStatus='$status',AssigntoName='$aname',AssignedtoEmpId='$pid',AssignedTime='$assigntime' where id='$testid'");
echo '<script>alert("Assigned to Pathologist successfully.")</script>';
...
$query2=mysqli_query($con,"update covidtestrecord set ReportStatus='$status',FinalReport='$newreportfile',ReportUploadTime='$uploadtime' where OrderNumber='$orderid'");
...
$query=mysqli_query($con,"insert into reporttracking(OrderNumber,Status,Remark,RemarkBy) values('$orderid','$status','$remark','$rby')");
$query2=mysqli_query($con,"update covidtestrecord set ReportStatus='$status' where OrderNumber='$orderid'");
echo '<script>alert("Status updated successfully")</script>';
echo "<script>window.location.href='all-test.php'</script>";
endif;  
...
<?php 
$testid=intval($_GET['tid']);
$query=mysqli_query($con,"select * from covidtestrecord
join covidpatient on covidpatient.MobileNumber=covidtestrecord.PatientMobileNumber
where  covidtestrecord.id='$testid'");
...
join covidadmin on covidadmin.ID=reporttracking.RemarkBy
...
<form method="post">
          <p>  <select class="form-control" name="assignto" required="true">
            ...
            <?php $sql=mysqli_query($con,"select FullName,EmpID from pathologist");
...
<form method="post" enctype="multipart/form-data" >
  ...
  $query1=mysqli_query($con,"select ReportStatus from covidtestrecord where OrderNumber='$orderid'");
